// HoughConvert.cpp : このファイルには 'main' 関数が含まれています。プログラム実行の開始と終了がそこで行われます。
//
#include <vector>
#include <tuple>
#include <iostream>
...
int main()
{
    ...
    vector<pair<long, long>> l_R;
    vector<pair<long, long>> l_R2;
    vector<float> X;
    vector<float> Y;
    int point = 10;
    for (int i = -90;i < 90;i++)
    {
        ...
        //    Y.push_back(2 * i);
        //}
        l_R.emplace_back(50 * cos(i * 3.14 / 180), 50 * sin(i * 3.14 / 180));
    }
   


    vector < tuple<int, int, int >> Sum;
    vector < tuple<float, int, int >> debug;
    vector < tuple<float, int, int >> filtered;
    vector<pair<int, int>> lpf;
    vector<pair<int, int>> Vpeaks;
